// Problema 2.2.1: f(x)=e^-x - sen(x) com erro = 1e-05
...
x=0:0.1:1      // define onde começa e termina o eixo das abscissas
y=exp(-x)-sin(x)     // função padrão
...
xtitle("Localização das raízes: gráfico de f(x) = e^-x - sen(x)",...
       "Valores do domínio [0 , 1]",...
...
deff('y = f(x)', 'y = exp(-x)-sin(x)')           // definindo a função padrão
...
deff('y = f(x)', 'y = exp(-x)-sin(x)')                       // definindo a função 
deff('y = f1(x)', 'y = -exp(-x)-cos(x)')                           // definindo a derivada (primeira) da função
old_x = 0.5                                            // condição inicial do problema (xk); xm de [o,5; 1]
